...
io.on("connection", (socket) => {
  ...
  socket.on("send_message", (message) => {
    ...
    const timestamp = new Date().toISOString(); // Add timestamp to message
    const messageWithTimestamp = { ...message, timestamp }; // Add timestamp to message object

    socket.to(roomCode).emit("new_message", messageWithTimestamp); // Broadcast to all users except sender
    socket.emit("new_message", messageWithTimestamp); // Send back to sender
  });
